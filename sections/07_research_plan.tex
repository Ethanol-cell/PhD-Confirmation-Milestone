...
\midrule
The tested CNNs contain a late representation-transition regime
& Train-fitted PCA-RGB; calibration-only selector; trained--random and
label-permutation controls; Pre-LN/Post-LN mechanism audit; CIFAR-10,
CIFAR-100, and ImageNet-1K confirmation
& Quantitatively supported for the tested ConvNeXt settings; additional
architectures and trained seeds are required before claiming a universal
transition location.\\
\thinrowrule
Adaptive robust prototype-relative geometry localizes object-relevant spatial
structure
& Original 600-image regression and separate 580-image primary confirmatory
localization; verified prototype and CDF assets; exact distance decomposition;
prospective 1,000-image RANDOM64 geometry-preservation control
& Supported for class-agnostic object-relevant localization in the stated CNN
cohorts. RANDOM64 supports distance preservation as mechanism evidence, but its
median end-to-end speedup is only $0.999\times$; RAW Adaptive K-DGRP remains
canonical.\\
\thinrowrule
Full Progress II faithfulness and sanity closure
& Four of six ImageNet-100 mask-free model outputs are present; the 890-image
prospective curves require historical-identity verification; ROAD and
parameter/label randomization are incomplete
& Open. Probability-based AUC, historical content identity, final prospective
statistics, ROAD, randomization, and the semantic/error atlas must pass their
frozen gates before Progress II is called final.\\
\thinrowrule
...
\jmethod{} paper.} The PCA-RGB study already has a frozen display protocol, a
calibration-only transition selector, trained--random and permutation controls,
wrong-prediction analysis, mechanism controls, and cross-dataset evidence.
Remaining work is limited to the replication needed to define the scope of the
transition claim.

\textbf{The current evidence is sufficient to retain RAW Adaptive K-DGRP and
close the search over alternative scoring formulas, but Progress II is not yet
final.} The remaining K-DGRP work is specifically bounded: verify historical
content identity for saved curves; replace logit-integrated AUC with frozen
target-class probability AUC; complete ROAD; finish parameter and label
randomization; produce the final prospective statistics; and freeze a
qualitative semantic/error atlas. External-domain generalization can then
define the scope of the method. This work should not reopen an unbounded search
over PCA, radius-normalized, common-gain, or residual scoring formulas. The J1
paper still requires replication of the strongest external baselines, an
uncertainty-aware J1--ReviewKD comparison, resource-cost reporting, and a
generality test beyond one teacher--student setting.

\subsection{Transformer investigation across a broad design space}

After the CNN work is complete, the transformer literature review will begin
with a broad comparison of the objects explained by attention rollout,
gradient-weighted attention, relevance propagation, token-transformation
methods, and feature-formation methods
\cite{abnar2020attentionflow,chefer2021transformer,
...
a small reproducible baseline set and identify what can be compared fairly with
the CNN representation study.

The first pilot will not assume that a CNN feature grid has a direct
transformer equivalent. Candidate objects include patch tokens before and after
attention, residual-stream states, value vectors, attention-output features,
and hierarchical stage outputs. Candidate architectures may include a flat
supervised Vision Transformer (ViT), a data-efficient variant, and a
hierarchical transformer. Reproducible probes and computational feasibility
...
(CKA), linear probes, affinity change, token clustering, or class-prototype
geometry. If \methodname{} does not transfer directly, that negative result
will identify which CNN assumptions---local spatial grids, convolutional common
modes, or Euclidean channel geometry---do not survive token mixing.

\subsection{Two-year schedule}

\begin{figure}[H]
\centering
\resizebox{\linewidth}{!}{\input{figures/gantt}}
...
representation geometry. The third should test the scope of the thesis
hypothesis rather than introduce an unrelated transformer method.
...
Another risk is that \methodname{} remains strong for localization but weak for
class causality. The contribution should then be positioned explicitly as
