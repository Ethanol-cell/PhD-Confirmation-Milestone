...
continuing unbounded formula search. The PCA-RGB study now has a frozen display protocol, a
...
Remaining work is limited to cross-architecture and trained-seed replication
needed to define the scope of the transition claim. The \methodname{} paper needs a prospective cross-dataset study
with the mode-count selector frozen before segmentation evaluation. It also
needs expanded faithfulness and sanity tests, together with a fixed semantic
atlas that includes failures. The J1 paper needs replication of the strongest
external baselines, an uncertainty-aware J1--ReviewKD comparison, resource-cost
reporting, and a generality test beyond one teacher--student setting.
...
a flat supervised Vision Transformer (ViT), a data-efficient variant, and a
